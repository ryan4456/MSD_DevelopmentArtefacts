			...
			<view class="filter-content" v-for="(item, index) in menuList" :key="index" v-if="menuIndex == index">
				<view v-if="item.isSort">
					<view class="filter-content-list">
						<view v-for="(detailItem,idx) in selectDetailList" :key="idx" :class="detailItem.isSelected?'filter-content-list-item-active':'filter-content-list-item-default'"
						 :style="{'color': detailItem.isSelected?themeColor:'#666666', 'border-bottom': idx == selectDetailList.length - 1 ? 'none' : ''}" @tap="sortTap(idx,selectDetailList,item.key)">
				...
				<view v-else>
					...
					<view class="filter-content-detail">
						<text v-for="(detailItem,idx) in selectDetailList" :key="idx" v-if="idx < item.length1" class='filter-content-detail-item-default' :style="{'background-color':detailItem.isSelected?themeColor:'#F6F6F6','color':detailItem.isSelected?'#FFFFFF':'#666666'}"
						 @tap="itemTap(idx,selectDetailList,item.isMutiple1,item.key, item.length1)">
							{{detailItem.title}}
						</text>
					</view>
					<view class="filter-content-title" v-if="item.detailTitle2 && item.detailTitle2.length">
						<text>{{item.detailTitle2}}</text>
					</view>
					<view class="filter-content-detail">
						<text v-for="(detailItem,idx) in selectDetailList" :key="idx" v-if="idx >= item.length1" class='filter-content-detail-item-default' :style="{'background-color':detailItem.isSelected?themeColor:'#F6F6F6','color':detailItem.isSelected?'#FFFFFF':'#666666'}"
						 @tap="itemTap(idx,selectDetailList,item.isMutiple2,item.key, item.length1)">
							{{detailItem.title}}
						</text>
					</view>
...
<script>
	export default {
		...
		methods: {
			getSelectedObj() {
				...
				for (let i = 0; i < this.menuList.length; i++) {
					...
					if (!this.independence && item.defaultSelectedIndex != null && item.defaultSelectedIndex.toString().length > 0) { // 处理并列菜单默认值
						...
						if (item.isMutiple || item.isMutiple1 || item.isMutiple2) {
							obj[item.key] = [];
			...
			itemTap(index, list, isMutiple, key, length) {
				if (isMutiple == true) {
					...
					if (index == -1) {
						...
						// 		if(_index < length && _index != index){
						// 			_item.isSelected = false;
						// 			let idx = this.selectedObj[key].indexOf(list[_index].value);
